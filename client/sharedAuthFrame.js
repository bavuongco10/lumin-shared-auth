...
let catchWrap = function(callable, thisval) {
    return function() {
      try {
...
Template.sharedAuthFrame.helpers({
  'sharedAuthFrame'() {
    ...
    let notifyParent = catchWrap(function notifyParent() {
    ...
    let onMessage = catchWrap(function onMessage(event) {
      // Ensure we are within allowed origins.
      let found = false;
      for (let i = 0; i < Meteor.settings.public.sharedAuthDomains.length; i++) {
    ...
    let onStorageChange = catchWrap(function onStorageChange(event) {
